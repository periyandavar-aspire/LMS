function autocomplete(inputElem, destination, url) {
    var currentFocus;
    inputElem.addEventListener('input', function(e) {
        ...
        let selected = destination.getElementsByTagName('input')[0].value;
        fetch(url + val + "/" + selected, { headers: { response: "application/json" } })
            .then(response => { return response.json() })
            .then(data => {
                for (i = 0; i < data.result.length; i++) {
                    innerDiv = document.createElement("div");
                    innerDiv.innerHTML += "" + data.result[i]['value'].substr(0, val.length) + "" + data.result[i]['value'].substr(val.length);
                    ...
                    innerDiv.addEventListener("click", function(e) {
                        let selectedValues = destination.getElementsByTagName('input')[0].value;
